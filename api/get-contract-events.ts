...
// DESCRIZIONE: Versione corretta con l'ABI esatto fornito dall'utente e un filtro di sicurezza.

import type { VercelRequest, VercelResponse } from '@vercel/node';
import { createThirdwebClient, getContract, getContractEvents } from 'thirdweb';
import { polygon } from 'thirdweb/chains';

// --- MODIFICA CHIAVE 1: Utilizzo dell'ABI esatto fornito dall'utente ---
...
export default async function handler(
  ...
  try {
    ...
    // --- MODIFICA CHIAVE 2: Filtro manuale con controllo di sicurezza ---
    const userBatches = allBatchInitEvents.filter(event => {
      const contributor = (event.data as any)?.contributor;
      // Controlla che il campo esista prima di chiamare toLowerCase()
