...
export default async function handler(
  ...
  try {
    ...
    // --- MODIFICA 1: Recupera TUTTI gli eventi necessari in parallelo ---
    ...
    // --- MODIFICA 2: Filtra e processa i dati MANUALMENTE sul server ---
    
    // Filtra i batch per l'utente loggato
    const userBatches = allBatchInitEvents.filter(event => 
      (event.data as any).contributor.toLowerCase() === userAddress.toLowerCase()
    );

    // Raggruppa gli step per batchId per un accesso rapido
    const stepsByBatchId = new Map<string, any[]>();
    for (const stepEvent of allBatchStepEvents) {
      const batchId = (stepEvent.data as any).batchId.toString();
      if (!stepsByBatchId.has(batchId)) {
        stepsByBatchId.set(batchId, []);
      }
      stepsByBatchId.get(batchId)!.push(stepEvent.data);
    }
    
    // Combina i dati: aggiungi gli step a ogni batch dell'utente
    const combinedData = userBatches.map(batchEvent => {
      ...
      return {
        ...
        steps: stepsByBatchId.get(batchId) || [] // Aggiunge gli step, o un array vuoto se non ce ne sono
      };
    });

    const serializableData = serializeBigInts(combinedData);
    
    // Invia i dati combinati e filtrati al frontend
