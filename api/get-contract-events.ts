...
// DESCRIZIONE: Versione finale con un controllo di sicurezza per prevenire crash
// dovuti a eventi malformati.

import type { VercelRequest, VercelResponse } from '@vercel/node';
import { createThirdwebClient, getContract, getContractEvents } from 'thirdweb';
import { polygon } from 'thirdweb/chains';

// Utilizzo dell'ABI esatto fornito dall'utente
...
export default async function handler(
  ...
  try {
    ...
    for (const stepEvent of allBatchStepEvents) {
      ...
      // --- MODIFICA CHIAVE: Controllo di sicurezza per prevenire il crash ---
      if (stepData && typeof stepData.batchId !== 'undefined' && stepData.batchId !== null) {
        ...
        // Opzionale: logga un avviso se un evento è malformato, senza far crashare l'app
        console.warn("Ignorato un evento BatchStepAdded malformato o senza batchId:", stepEvent);
      }
    }
